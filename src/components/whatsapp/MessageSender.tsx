...
export function MessageSender({ sessionId, onSessionChange }: MessageSenderProps) {
  ...
  const uploadFile = async (file: File): Promise<string> => {
    ...
    console.log('📁 [UPLOAD DEBUG] Tamaño del archivo:', (file.size / 1024 / 1024).toFixed(2), 'MB')
    ...
    try {
      ...
      const uploadUrl = 'http://100.42.185.2:8015/upload2'
      ...
      if (!data.imagePath) {
        ...
        console.error('📁 [UPLOAD DEBUG] Keys en respuesta:', Object.keys(data))
      ...
      console.log('📁 [UPLOAD DEBUG] Usando path local del backend:', imagePath)
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      console.log('🚀 [SEND DEBUG] Destinatario formateado:', formattedRecipient)
      ...
      switch (messageType) {
        ...
        case 'image':
          ...
          console.log('📤 [SEND DEBUG] Path local de imagen del backend:', imageUrl)
          ...
          console.log('📤 [SEND DEBUG] Payload imagen:', JSON.stringify(imagePayload, null, 2))
          console.log('📤 [SEND DEBUG] Llamando baileysAPI.sendImageMessage con sessionId:', sessionId)
        ...
        case 'video':
          ...
          console.log('📤 [SEND DEBUG] Path local de video del backend:', videoUrl)
          ...
          console.log('📤 [SEND DEBUG] Payload video:', JSON.stringify(videoPayload, null, 2))
        ...
        case 'audio':
          ...
          console.log('📤 [SEND DEBUG] Path local de audio del backend:', audioUrl)
          ...
          console.log('📤 [SEND DEBUG] Payload audio:', JSON.stringify(audioPayload, null, 2))
        ...
        case 'document':
          ...
          console.log('📤 [SEND DEBUG] Procesando documento:', selectedFile.name)
          const docUrl = await uploadFile(selectedFile)
          console.log('📤 [SEND DEBUG] Path local de documento del backend:', docUrl)
          ...
          console.log('📤 [SEND DEBUG] Payload documento:', JSON.stringify(docPayload, null, 2))
          response = await baileysAPI.sendDocumentMessage(sessionId, docPayload)
          break
      ...
      console.error('❌ [SEND DEBUG] Error stack:', error instanceof Error ? error.stack : 'No stack')
      console.error('❌ [SEND DEBUG] Error message:', error instanceof Error ? error.message : 'No message')
  ...
  const getMessageTypeIcon = (type: MessageType) => {
    switch (type) {
      ...
      case 'poll': return <Smile className="h-4 w-4" />
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Select value={isGroup ? 'group' : 'individual'} onValueChange={(value) => setIsGroup(value === 'group')}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
