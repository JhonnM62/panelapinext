...
interface AnalyticsData {
  ...
  trends: {
    ...
    dailyActivity: { date: string; sessionCount: number; webhookCount: number }[]
  ...
  recentActivity: {
    id: string
    type: 'session_created' | 'session_connected' | 'session_disconnected' | 'webhook_triggered' | 'error'
    title: string
    description: string
    timestamp: string
...
export default function AnalyticsPage() {
  ...
  const loadAnalyticsData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const [sessionsResponse, healthResponse, userToken] = await Promise.allSettled([
        sessionsAPI.list(),
        utilsAPI.getHealth(),
      ...
      if (sessionsResponse.status === 'fulfilled' && sessionsResponse.value.status) {
        sessions = sessionsResponse.value.data || []
      }

      // Procesar respuesta de health
      if (healthResponse.status === 'fulfilled' && healthResponse.value.status) {
      ...
      try {
        ...
        console.warn('No se pudieron cargar las estadísticas de webhooks:', error)
      }

      // Procesar sesiones para analytics
      const processedSessions: SessionAnalytics[] = sessions.map(session => ({
      ...
      const authenticatedSessions = processedSessions.filter(s => 
      ...
      const sessionStatusTrends = Object.entries(statusDistribution).map(([status, count]) => ({
      ...
      const recentActivity = [
        ...processedSessions.slice(0, 3).map(session => ({
          ...
          type: session.status === 'authenticated' ? 'session_connected' : 'session_created' as const,
          title: `Sesión ${session.status === 'authenticated' ? 'autenticada' : 'creada'}`,
          description: `Sesión ${session.id} ${session.status === 'authenticated' ? 'se conectó exitosamente' : 'fue creada'}`,
          timestamp: session.lastActivity || new Date().toISOString(),
          status: session.status === 'authenticated' ? 'success' : 'warning' as const,
        ...
        })),
        {
          ...
          title: 'Health Check',
          description: `Servidor funcionando correctamente. Memoria: ${health?.memory?.used || 'N/A'}`,
      ...
      const finalAnalyticsData: AnalyticsData = {
        overview: {
          ...
          errorRate: processedSessions.reduce((sum, s) => sum + (s.errorCount || 0), 0)
        },
        trends: {
  ...
  const exportData = () => {
    ...
    a.href = url
    a.download = `whatsapp_analytics_${timeRange}_${new Date().toISOString().split('T')[0]}.json`
    a.click()
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-300 mt-2">
            Monitoreo en tiempo real de sesiones, webhooks y rendimiento del sistema
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-4">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{analyticsData.overview.activeSessions}</p>
              </div>
        ...
        <Card className="hover:shadow-lg transition-shadow">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Autenticadas</p>
                <p className="text-2xl font-bold text-emerald-600">{analyticsData.overview.authenticatedSessions}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{analyticsData.overview.totalWebhooks}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">{analyticsData.overview.memoryUsage}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">{analyticsData.overview.errorRate}</p>
              </div>
          ...
          <CardContent>
            <div className="h-64 flex items-end justify-between space-x-2 px-2">
              {analyticsData.trends.dailyActivity.map((day, index) => {
                const maxCount = Math.max(...analyticsData.trends.dailyActivity.map(d => d.sessionCount + d.webhookCount))
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {session.messageCount} mensajes • {session.chatCount} chats
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <Badge variant={
                        session.status === 'authenticated' ? 'default' :
                        session.status === 'connected' ? 'secondary' :
                        session.status === 'connecting' ? 'outline' : 'destructive'
                      }>
                        {session.status}
                      </Badge>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">{analyticsData.health.status.toUpperCase()}</span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-lg font-semibold">
                  {analyticsData.health.memory.used} / {analyticsData.health.memory.total}
                </p>
              </div>
