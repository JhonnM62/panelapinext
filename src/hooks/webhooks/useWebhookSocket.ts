  ...
  return {
    ...
    connect: async (url: string): Promise<WebSocket> => {
      ...
      if (reconnectAttempts >= maxReconnectAttempts) {
        throw new Error(`Máximo de intentos de reconexión alcanzado (${maxReconnectAttempts})`);
      }
      
      isConnecting = true;
      connectionId = `conn_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      
      console.log(`[WS MANAGER] 🚀 Iniciando conexión (ID: ${connectionId}, intento: ${reconnectAttempts + 1})`);
      ...
      connectionPromise = new Promise((resolve, reject) => {
        try {
          ...
          ws.onopen = () => {
            ...
            console.log(`[WS MANAGER] ✅ Conexión establecida (ID: ${connectionId})`);
          ...
          ws.onerror = (error) => {
            console.error(`[WS MANAGER] ❌ Error de conexión (ID: ${connectionId}):`, error);
            clearTimeout(connectionTimeout);
            connectionPromise = null;
            isConnecting = false;
            reconnectAttempts++;
            ...
            subscribers.forEach(callback => {
              try {
                ...
                console.warn('[WS MANAGER] Error en subscriber onerror:', error);
              }
            });
          ...
          ws.onclose = (event) => {
            console.log(`[WS MANAGER] 🔴 Conexión cerrada (ID: ${connectionId})`, {
              code: event.code, 
              reason: event.reason,
            ...
            subscribers.forEach(callback => {
              try {
                ...
                console.warn('[WS MANAGER] Error en subscriber onclose:', error);
              }
            });
          };
          ...
          console.error(`[WS MANAGER] ❌ Excepción creando WebSocket (ID: ${connectionId}):`, error);
          connectionPromise = null;
          isConnecting = false;
          reconnectAttempts++;
          reject(error);
    ...
    authenticate: (userId: string) => {
      if (instance && instance.readyState === WebSocket.OPEN && !isAuthenticated) {
...
export const useWebhookSocket = (options: UseWebhookSocketOptions) => {
  ...
  const handleNewNotification = useCallback((notification: NotificationItem) => {
    if (!notification.id || !notification.eventType) {
      console.warn('[WEBHOOK SOCKET] ⚠️ Notificación inválida:', notification);
      return;
    }
    
    // Verificar duplicados
    if (processedNotificationIds.current.has(notification.id)) {
      console.log('[WEBHOOK SOCKET] 🚫 Notificación duplicada:', notification.id);
      return;
    }
    ...
    websocket.onmessage = (event) => {
      ...
      try {
        ...
        switch (message.type) {
          ...
          case "notifications":
            console.log("[WEBHOOK SOCKET] 📋 Lista de notificaciones recibida");
            break;
            
          case "error":
            const errorMsg = message.message || message.error || 'Error desconocido del servidor';
          ...
          default:
            console.log("[WEBHOOK SOCKET] 📋 Mensaje no manejado:", message.type);
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const websocket = await globalWebSocketManager.connect("ws://100.42.185.2:8015/ws");
  ...
  useEffect(() => {
    ...
    const unsubscribe = globalWebSocketManager.subscribe((websocket, connected) => {
      setWs(websocket);
