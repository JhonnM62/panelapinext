...
export const usePlanLimits = () => {
  ...
  const calculateLimitValidation = useCallback((current: number, limit: number): LimitValidation => {
    ...
    return {
      ...
      remaining,
      percentage,
      message: current >= limit ? 'Límite alcanzado' : 
               percentage >= 90 ? 'Cerca del límite' :
               percentage >= 70 ? 'Uso moderado' : 'Disponible'
    }
  }, [])
  ...
  const canCreateResource = useCallback((type: ResourceType): boolean => {
  ...
  const getResourceUsage = useCallback((type: ResourceType): LimitValidation | null => {
    const limits = getResourceLimits()
  ...
  const incrementResourceUsage = useCallback(async (type: ResourceType) => {
  ...
  const decrementResourceUsage = useCallback(async (type: ResourceType) => {
    if (!suscripcion) return false
  ...
  const validateResourceCreation = useCallback((type: ResourceType): { 
    canCreate: boolean
    reason?: string 
  } => {
    if (!user) {
    ...
    const limits = getResourceLimits()
    if (!limits) {
      return { canCreate: false, reason: 'No se pudieron obtener los límites' }
    }
  ...
  const createResource = useCallback(async (params: CreateResourceParams): Promise<{
    success: boolean
    data?: any
    error?: string
  }> => {
    ...
    try {
      ...
      switch (params.type) {
        case 'webhooks':
          ...
          result = await fetch('http://100.42.185.2:8015/webhook/create', {
            method: 'POST',
        ...
        case 'botsIA':
          ...
          result = await fetch('http://100.42.185.2:8015/chatbot/create', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              userId: user?.nombrebot,
