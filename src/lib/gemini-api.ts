...
class EnhancedBaileysAPI {
  ...
  private async request<T>(
    ...
    try {
      ...
      const hasAuthHeader = !!(
        mergedHeaders["Authorization"] || mergedHeaders["x-access-token"]
      );
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error(
            "🔧 [GEMINI API] Could not parse error response:",
            parseError
          );
        }
  ...
  async getGeminiConfig(
    token: string
  ): Promise<{ success: boolean; data?: GeminiConfigData; message?: string }> {
    try {
      console.log(
        "🔧 [GEMINI API] Obteniendo configuración con token:",
        token ? "TOKEN_PRESENTE" : "NO_TOKEN"
      );
  ...
  async saveGeminiConfig(params: {
    token: string;
    config: Partial<GeminiConfigData>;
  }): Promise<{ success: boolean; data?: any; message?: string }> {
    try {
      console.log(
        "🔧 [GEMINI API] Guardando configuración con token:",
        params.token ? "TOKEN_PRESENTE" : "NO_TOKEN"
      );
  ...
  async deleteGeminiConfig(params: {
    token: string;
    botId?: string;
    sesionId?: string;
  }): Promise<{ success: boolean; data?: any; message?: string }> {
    try {
      console.log(
        "🔧 [GEMINI API] Eliminando configuración con token:",
        params.token ? "TOKEN_PRESENTE" : "NO_TOKEN"
      );
      ...
      console.log(
        "🔧 [GEMINI API] Configuración eliminada exitosamente:",
        response.data
      );
  ...
  async processIADirect(data: ProcessIARequest): Promise<ProcessIAResponse> {
    try {
      console.log(
        "🔧 [GEMINI API] Iniciando procesamiento IA con token:",
        !!data.token
      );
      ...
      if (data.token) {
        ...
        console.log(
          "🔧 [GEMINI API] Token agregado a headers para autenticación"
        );
      } else {
        console.warn(
          "🔧 [GEMINI API] ⚠️ No se proporcionó token - la petición fallará"
        );
      }
  ...
  async getAdvancedStats(
    ...
    try {
      ...
      return {
        ...
        data: response.data,
        message:
          response.message || "Estadísticas avanzadas obtenidas exitosamente",
      };
    } catch (error) {
