...
export interface ServerConfigStore {
  ...
  testConnection: (url: string, type: 'backend' | 'python' | 'webhook', token: string) => Promise<ServerTestResult>;
...
const defaultServerConfig: ServerConfig = {
  backendUrl: 'http://100.42.185.2:8015',
  pythonApiUrl: 'http://100.42.185.2:8014',
  webhookUrl: 'http://100.42.185.2:8015/webhook',
  customUrls: {
    enabled: false,
    backendCustom: '',
    pythonApiCustom: '',
...
export const useServerConfigStore = create<ServerConfigStore>()(
  persist(
    (set, get) => ({
        ...
        try {
          ...
          if (result.success) {
            ...
            set({
              config: defaultServerConfig,
              isLoading: false,
              error: result.message || 'Error cargando configuración de servidores',
            });
          }
        } catch (error) {
      ...
      testConnection: async (url: string, type: 'backend' | 'python' | 'webhook', token: string) => {
      ...
      resetConfig: async (token: string) => {
        ...
        try {
          const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://100.42.185.2:8015';
          const response = await fetch(`${API_URL}/api/v2/server-config/reset`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
